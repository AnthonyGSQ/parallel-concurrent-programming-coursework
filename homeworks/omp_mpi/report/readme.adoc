...
Por temas de tiempo no pude recopilar los datos de la comparativa con y sin MPI del job020 entero, sin embargo, se espera que la version con MPI sea mas rapida que las versiones sin MPI, pues MPI permite el procesamiento de multiples laminas en paralelo, mientras que tareas anteriores solo permiten el procesamiento de una lamina a la vez. La tabla a continuacion muestra los resultados de la comparativa entre OpenMP, pthreads y serial con la primera lamina del job20.

[%autowidth.stretch,options="header"]
|===
|Etiqueta   |Hilos |Duración (s) |_Speedup_ |Eficiencia |Descripción corta
|Serial     |-     |540.319      |1.00      |100%       |Versión serial final (Tarea01)
|Pthread    |4     |228.759      |2.36      |59.0%      |Versión concurrente con pthreads (Tarea02)
|Optimized  |4     |84.410       |6.40      |160.1%     |Versión concurrente con pthreads optimizada (Tarea03)
|Omp_mpi    |4     |120.341      |4.49      |112.4%     |Versión concurrente con OpenMP y MPI (Tarea04)
|===