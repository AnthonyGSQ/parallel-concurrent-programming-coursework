...
int create_threads(shared_data_t* shared_data) {
  ...
  if (threads && private_data) {
    ...
    for (uint64_t thread_number = 0; thread_number < shared_data->thread_count
      ...
      // create_thread(race, thread_number)
...
void* race(void* data) {
  ...
  // bloqueamos una parte del codigo para que sea ejecutada por un
  // solo hilo a la vez
  pthread_mutex_lock(&shared_data->can_access_position);

  // race condition/ data race/ condicion de carrera
  // modificacion concurrente de memoria compartida
  ...
  printf("Thread %" PRIu64 "/ %" PRIu64 ": I arrived at position %" PRIu64 "\n"
    , private_data->thread_number, shared_data->thread_count, shared_data->position);
